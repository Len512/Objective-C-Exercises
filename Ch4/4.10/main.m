...
int main(int argc, const char * argv[]) {
    @autoreleasepool {
        ...
        NSLog(@"Store %g", [myCalc memoryStore]);
        NSLog(@"MAdd %g", [myCalc memoryAdd:34]);
        NSLog(@"Recall %g", [myCalc memoryRecall]);
        NSLog(@"MSubtract %g", [myCalc memorySubtract:34]);
        NSLog(@"Recall %g", [myCalc memoryRecall]);
        NSLog(@"Clear %g", [myCalc memoryClear]);
    }
    return 0;
}
